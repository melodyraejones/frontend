...
class MyCart {
  ...
  async addProductFromDetails(button) {
    ...
    const title = productElement.querySelector(".heading-primary").textContent.trim();
  ...
  initializeProductQuantities() {
    ...
    programElements.forEach(programElement => {
      ...
      if (productId) {
        ...
        console.error("data-id attribute is missing or incorrect in .program element");
      }
    });
  }
  ...
  async addItemToCart(item, button, programElement) {
    try {
      ...
      if (response.data && response.data.cartItemId) {
        ...
        console.error("Failed to add item to the cart: No cart item ID received from the backend.");
      }
    } catch (e) {
  ...
  async loadCartItems() {
    try {
      ...
      this.cartItems = response.data.map(item => ({
        ...
        quantity: item.program_quantity,
        // Assuming this is how the backend sends the quantity
      ...
      this.cartItems.forEach(item => {
        const addButton = document.querySelector(`.add_to_cart[data-id="${item.productId}"]`);
        const quantityElement = document.querySelector(`.program[data-id="${item.productId}"] .product-quantity`);
        if (addButton) {
  ...
  async removeItemFromCart(itemOrTitle, itemId) {
    let itemToRemove = this.cartItems.find(item => item.id === itemId || this.normalizeTitle(item.title) === this.normalizeTitle(itemOrTitle));
    if (!itemToRemove) {
      console.error("Item to remove is missing an ID or title");
      return;
    }
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].delete(`${mrjData.root_url}/wp-json/wp/v2/cart/${itemToRemove.id}`, {
        headers: {
          "X-WP-Nonce": mrjData.nonce
        }
      });
  ...
  async updateTotalOnBackend() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].get(`${mrjData.root_url}/wp-json/mrj/v1/cart-total`, {
        headers: {
          "X-WP-Nonce": mrjData.nonce,
          "Content-Type": "application/json"
        }
  ...
  async checkout() {
    try {
      const cartTotalResponse = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].get(`${mrjData.root_url}/wp-json/mrj/v1/cart-total`, {
        headers: {
          "X-WP-Nonce": mrjData.nonce,
          "Content-Type": "application/json"
        }
      ...
      if (cartTotalResponse.data && cartTotalResponse.data.items.length > 0) {
        ...
        const checkoutResponse = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(`${mrjData.root_url}/wp-json/mrj/v1/checkout`, checkoutData, {
          headers: {
            "X-WP-Nonce": mrjData.nonce,
            "Content-Type": "application/json"
          }
        ...
        console.error("Failed to fetch cart total or no items in cart:", cartTotalResponse.data);
      }
    } catch (error) {
...
/***/ "./src/modules/direct-checkout.js":
/*!****************************************!*\
  !*** ./src/modules/direct-checkout.js ***!
  \****************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   initializeDirectCheckout: () => (/* binding */ initializeDirectCheckout)
/* harmony export */ });
...
/* harmony import */ var _modules_audio__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/audio */ "./src/modules/audio.js");
...
/* harmony import */ var _modules_direct_checkout__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modules/direct-checkout */ "./src/modules/direct-checkout.js");
// Our modules / classes
